...
def _worker_target(doc):
  # logging.info('entered _worker_target')
  # logging.info(f'curr global worker target: {_GLOBAL_WORKER_TARGET}')
...
def worker_target_factory(
  ...
  # logging.info('entered worker_target_factory (good)')
  
  def fn(doc_and_char_masks):
    # logging.info('entered fn')
    doc, char_masks = doc_and_char_masks
    try:
      # logging.info('before doc_and_char_masks_to_input_and_tt')
...
def doc_and_char_masks_to_input_and_tt(
    ...
    sequence_length,
    skip_naive_incomplete):
  # print('entered doc_and_char_masks_to_input_and_tt', flush=True)
  # logging.info('entered doc_and_char_masks_to_input_and_tt')
  # Tokenize document
  try:
  ...
  # print('OUTPUT OF DOC_AND_CHAR_MASKS...:\n', inputs, '\n', tts)
  # input()
...
def masked_dataset_to_inputs_and_tts(
  ...
  _GLOBAL_WORKER_TARGET = worker_target_factory(
      ...
      skip_naive_incomplete)
  # logging.info(f'_GLOBAL_WORKER_TARGET: {_GLOBAL_WORKER_TARGET}')
  # print('check:')
  # input(_GLOBAL_WORKER_TARGET)
  ...
  #   logging.info('entered multiprocessing')
  #   docs_inputs_and_tts = list(tqdm(
  #     p.imap(_worker_target, dataset),
  #     total=len(dataset)))
    
  docs_inputs_and_tts = [_worker_target(doc) for doc in dataset]
...
def train(args):
  ...
  # input(args.train_num_epochs)
  if args.train_num_epochs is not None:
  # if args.train_num_epochs is None:
  ...
  if args.eval_only:
    ...
    while True:
      ...
      for batch in train_dataloader:
        ...
        # print(f'epoch: {args.train_num_epochs}, batches complete: {num_batches_complete}', end='\r')
        # print(num_batches_complete)
        # print('*')
        ...
        if int(elapsed / args.train_eval_secs) > num_save:
          ...
          for i, eval_batch in enumerate(eval_dataloader):
            with torch.no_grad():
              eval_inputs, eval_tts = tuple(t.to(device) for t in eval_batch)
              # res = model(eval_inputs)
              # eval_logits = res.logits
              eval_logits, _ = model(eval_inputs)
              eval_logits_relevant = eval_logits[:, :-1].contiguous().view(-1, eval_logits.shape[-1])
          ...
          if best_eval_loss is None or eval_dict['eval_infill_loss'] < best_eval_loss:
            ...
            torch.save(model_to_save.state_dict(), out_fn_to_fp(WEIGHTS_NAME)) # pytorch_model.bin
