...
class MaskConjunctionType(Enum):
  CONJUNCTION = 0

class MaskConjunction(MaskFn): 
  def __init__(self, p=1.):
    try:
      pos_tag(['Ensure', 'tagger'])
    except:
  ...
  @classmethod
  def mask_types(cls):
    return list(MaskConjunctionType)
  ...
  def mask(self, doc):
    ...
    toks_pos = pos_tag(toks)
    for t, off, (_, pos) in zip(toks, toks_offsets, toks_pos):
      if pos == 'CC' and random.random() < self.p:
        masked_spans.append((MaskConjunctionType.CONJUNCTION, off, len(t)))
    return masked_spans
